  ...
  try {
    ...
    if (!shopName || !bankName || !bankOwner || !bankAccount) {
      return res.status(400).json({
        message:
          "Thiếu các trường bắt buộc: shopName, bankName, bankOwner, bankAccount",
      });
    }
    ...
    res.status(201).json({
      success: true,
      message:
        "Đăng ký đối tác thành công. Vui lòng chờ quản trị viên phê duyệt.",
      data: result,
    });
  } catch (error: any) {
    console.error("Error registering vendor:", error);
    if (
      error.message === "Bạn đã đăng ký trở thành đối tác rồi" ||
    ...
    res
      .status(500)
      .json({
        message: "Lỗi máy chủ khi đăng ký đối tác",
        error: error.message,
      });
  }
};
...
export const getVendorProfile = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy thông tin đối tác", error: error.message });
  }
};
...
export const updateVendorProfile = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    const { shopName, logo, description, bankName, bankOwner, bankAccount } =
      req.body;
    ...
    res
      .status(500)
      .json({
        message: "Lỗi khi cập nhật thông tin đối tác",
        error: error.message,
      });
  }
};
...
export const createVendorHotel = async (req: Request, res: Response) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Tạo khách sạn thành công, chờ duyệt",
        data: hotel,
      });
  } catch (error: any) {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi tạo khách sạn", error: error.message });
  }
};
...
export const deleteVendorHotel = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi xóa khách sạn", error: error.message });
  }
};
...
export const createVendorHotelRoom = async (req: Request, res: Response) => {
  try {
    ...
    if (!roomNumber || !type || price === undefined || capacity === undefined) {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp đủ thông tin bắt buộc của phòng" });
    }
    ...
    res
      .status(201)
      .json({ success: true, message: "Tạo phòng thành công", data: room });
  } catch (error: any) {
...
export const updateVendorHotelRoom = async (req: Request, res: Response) => {
  try {
    ...
    const room = await vendorsService.updateVendorHotelRoom(
      userId,
      hotelId,
      roomId,
      {
...
export const updateVendorBookingStatus = async (
  ...
  try {
    ...
    const updated = await vendorsService.updateVendorBookingStatus(
      userId,
      bookingId,
      status,
    );
    ...
    if (
      error.message === "Không tìm thấy hồ sơ đối tác" ||
...
export const getVendorRevenueReport = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy báo cáo doanh thu", error: error.message });
  }
};
...
export const getVendorReviews = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy danh sách bình luận", error: error.message });
  }
};
...
export const replyToReview = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi phản hồi bình luận", error: error.message });
  }
};
...
export const resetVendorProfile = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Hồ sơ đã được đặt lại thành công. Bạn có thể gửi lại yêu cầu đăng ký mới.",
    });
  } catch (error: any) {
    ...
    if (
      error.message === "Không tìm thấy hồ sơ đối tác" ||
      error.message ===
        "Chỉ có thể đặt lại hồ sơ khi yêu cầu bị từ chối phê duyệt"
    ) {
      return res.status(400).json({ message: error.message });
    }
    res
      .status(500)
      .json({ message: "Lỗi khi đặt lại hồ sơ đối tác", error: error.message });
  }
};
...
export const getVendorDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Lỗi khi lấy dữ liệu bảng điều khiển đối tác",
        error: error.message,
      });
  }
};
...
export const createWithdrawRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (!amount || amount < 100000) {
      return res.status(400).json({ message: "Số tiền rút tối thiểu là 100.000 VNĐ" });
    }

    const request = await vendorsService.createWithdrawRequest(userId, Number(amount), note);
    res.status(201).json({ success: true, message: "Tạo yêu cầu rút tiền thành công", data: request });
  } catch (error: any) {
...
export const updateVendorPromotion = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    const promoId = getIdParam(req.params.id);
    const promotion = await vendorsService.updateVendorPromotion(userId, promoId, req.body);
    res.status(200).json({ success: true, message: "Cập nhật khuyến mãi thành công", data: promotion });
  } catch (error: any) {
...
export const deleteVendorPromotion = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    const promoId = getIdParam(req.params.id);
    await vendorsService.deleteVendorPromotion(userId, promoId);
